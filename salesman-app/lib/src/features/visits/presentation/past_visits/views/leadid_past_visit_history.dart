...
class LeadIDPastVisitsHistoryView extends StatefulWidget {
  final int leadID;
  const LeadIDPastVisitsHistoryView({super.key, required this.leadID});

  @override
  State<LeadIDPastVisitsHistoryView> createState() =>
      _LeadIDPastVisitsHistoryViewState();
}

class _LeadIDPastVisitsHistoryViewState
    extends State<LeadIDPastVisitsHistoryView> {
  final ScrollController _scrollController = ScrollController();
  late final LeadIdPastVisitsCubit leadIDPastVisitsCubit = context
      .read<LeadIdPastVisitsCubit>();
  ...
  void initState() {
    super.initState();
    leadIDPastVisitsCubit.getLeadPastVisits(
      pageNumber: 1,
  ...
  void _loadMoreVisits() {
    if (leadIDPastVisitsCubit.state is LeadIDPastVisitsLoading &&
        !(leadIDPastVisitsCubit.state as LeadIDPastVisitsLoading).isFirstPage) {
      return;
    }
    if (_currentPage >=
        leadIDPastVisitsCubit.leadIDPastVisits.pagination.totalPages) {
      return;
    }
    _currentPage++;
    leadIDPastVisitsCubit.getLeadPastVisits(
      ...
      builder: (context, state) {
        ...
        return Column(
          children: [
                ...
                Expanded(
                  child: Row(
                    children: [
                      ...
                      if (state is LeadIDPastVisitsSuccess) ...[
                        const GapH(8.0),
                        Container(
                          ...
                          child: Text(
                            "${leadIDPastVisitsCubit.leadIDPastVisits.pagination.totalItems}",
            ...
            Expanded(
              child:
                  state is LeadIDPastVisitsSuccess ||
                      (state is LeadIDPastVisitsLoading && !state.isFirstPage)
  ...
  Widget _buildVisitsList(LeadIdPastVisitsState state) {
    final visits = leadIDPastVisitsCubit.leadIDPastVisits.visits;
    final hasMoreData =
        state is LeadIDPastVisitsSuccess &&
        _currentPage <
            leadIDPastVisitsCubit.leadIDPastVisits.pagination.totalPages;
